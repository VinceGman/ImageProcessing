// CREDIT : https://github.com/antimatter15/rgb-lab
...
Jimp.read(jpg)
    .then(image => {
        ...
        image.write(path, () => {
            fsExtra.rename(path, './images/original.jpg', function (err) {
                if (err) throw err
            });
            console.log("Original Successful!");
        });
    })
    .catch(err => {
...
function manipulate(rbg, cf, path) {
    ...
    // let tolerance = tol > 100 ? 100 : tol;
    // tolerance = tol < 0 ? 0 : tol;
        ...
        .then(image => {
            ...
            for (let x = 0; x < image.bitmap.width; x++) {
                for (let y = 0; y < image.bitmap.height; y++) {
                    let cur = [Jimp.intToRGBA(image.getPixelColor(x, y)).r, Jimp.intToRGBA(image.getPixelColor(x, y)).g, Jimp.intToRGBA(image.getPixelColor(x, y)).b];
                    let cur_lab = rgb2lab(cur);
                    if (deltaE(match_lab, cur_lab) < cf) {
